def generate_summary(filename, extra_info, plddt_data, type_df, uni_data):
    ...
    structures['IDs'] = ['example_protein', 'example_protein_2', 'example_protein_3', 'example_protein_4', 'example_protein_5']
    structures['mean_plddt'] = [78, 56, 123, 45, 345]
    structures = structures.sort_values(by=['mean_plddt'], ascending=False)
    structures_test = pd.concat([structures[:1], structures[-1:], structures[1:3]])
    structures_test = structures_test.reset_index(drop=True)
    structures_50 = structures_test
